...
let isAdult = confirm(`are you an adult or not`);
...
alert(`the sum is ${Number(numA) + Number(numB)}`);
alert(`the difference is ${Number(numA) - Number(numB)}`);
alert(`the product is ${Number(numA) * Number(numB)}`);
